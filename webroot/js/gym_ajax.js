...
$(document).ready(function () {
    ...
    $("body").on("click", ".add_category", function () {
        ...
        $.ajax({
            ...
            error: function (e) {
                alert(
                    "There was an error processing form.Please try again later."
                );
        ...
        if (confirm("Are you sure you want to delete this record?")) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var flash =
                        "<div class='message success' onclick=\"this.classList.add('hidden')\">Success! Record Deleted Successfully.</div>";
                    $(".content-wrapper").prepend(flash);
                },
                error: function (e) {
                    alert(
                        "There was an error deleting record,Please try again later."
                    );
                    console.log(e.responseText);
                },
            });
        ...
        if (confirm("Are you sure you want to delete this record?")) {
            $.ajax({
                ...
                success: function (response) {
                    $("#row-" + did).fadeOut("slow");
                    $(
                        ".specialization_list option[value=" + did + "]"
                    ).remove();
                    $("#specialization").multiselect("destroy");
                    $("#specialization").multiselect("rebuild");
                    var flash =
    ...
    $("body").on("change", "#pack_list", function () {
        var uid = $(this).val();
        var ajaxurl = $(this).attr("data-url"); /*getworkoutdates*/
        var curr_data = { uid: uid };
        if (uid != "") {
    ...
    $("body").on("change", ".gen_package_id", function () {
        var pid = $(this).val();
        $("#total_package_amount").val("");
        var ajaxurl = $(this).attr("data-url");
        var curr_data = { pid: pid };
        $.ajax({
            url: ajaxurl,
            data: curr_data,
            type: "POST",
            success: function (result) {
                $("#total_package_amount").val(result);
            },
        });
    });
