...
Class GymMemberController extends AppController
{
	...
	{	
		...
		$package = $this->GymMember->Package->find("list",["keyField"=>"id","valueField"=>"package_label"]);
		...
		if($this->request->is("post"))
		{
			...
			if($ext != 0)
			{
				...
				//package
				if(isset($this->request->data['package_valid_from']))
				{
					$this->request->data['package_valid_from'] = date("Y-m-d",strtotime($this->request->data['package_valid_from']));
				
				}
				if(isset($this->request->data['package_valid_to']))
				{
					$this->request->data['package_valid_to'] = date("Y-m-d",strtotime($this->request->data['package_valid_to']));
				}
				$this->request->data['first_package_pay_date'] = date("Y-m-d",strtotime($this->request->data['first_package_pay_date']));
				...
				{
					CASE "Member":
						$this->request->data['membership_status'] = "Active";
						$this->request->data['package_status'] = "Active";
					break;
					CASE "Prospect":
						$this->request->data['membership_status'] = "Not Available";
						$this->request->data['package_status'] = "Not Available";
					break;
					CASE "Non-member":
						$this->request->data['membership_status'] = "N/A";
						$this->request->data['package_status'] = "N/A";
					break;
					
				}
		/** 
				switch($this->request->data['package_type'])
				{
					CASE "Member":
						$this->request->data['package_status'] = "Active";
					break;
	...
	public function addPaymentHistory($data)
	{
		...
		$save["package_status"] = $data["package_status"];
