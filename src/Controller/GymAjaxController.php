...
Class GymAjaxController extends AppController
{
		...
		<div class="modal-header">
			...
			<div class="row">
				<div class="col-sm-12 table table-striped">
					...
					}else{echo "<tr id='empty_row'><td colspan='2'>".__("No Specialization added")."</td></tr>";}
					?>
					</tbody>
					</table>
				</div>
	...
	public function getPackageEndDate()
	{
		...
		$date = $this->request->data["date"];
		//$date = str_replace("/","-",$date);
		$package_id = $this->request->data["package"];
		$date2 = date("Y-m-d",strtotime($date));
		$package_table =  TableRegistry::get("Package");
		$row = $package_table->get($package_id)->toArray();
		$period = $row["package_length"];
		$package_end_date = date($format,strtotime($date2 . " + {$period} days"));
		echo $package_end_date;
	}
					...
					if(!empty($data))
					{
						foreach($data as $row)
						{
							echo "<tr id='row_{$row['id']}'>
									...
									<td>{$row['result']}</td>
									<td>".date($this->GYMFunction->getSettings("date_format"),strtotime($row['result_date']))."</td>
									<td>
		...
		<div class="modal-header">
			...
			<div class="row">
				<div class="col-sm-12 table table-striped">
					...
					}else{echo "<tr id='empty_row'><td colspan='2'>".__("No Event Place added")."</td></tr>";}
					?>
					</tbody>
					</table>
				</div>
	...
	public function getAmountByPackages()
	{
		if($this->request->is('ajax'))
		{
			$pid = $this->request->data['pid'];
			$pack_tbl = TableRegistry::get("Package");
			$row = $pack_tbl->get($pid)->toArray();
	...
	public function viewInvoice($mp_id)
	{
		...
		<thead>
			...
			<tbody>
				...
				{?>
					<tr>
					<td class="text-center"><?php echo date($this->GYMFunction->getSettings("date_format"),strtotime($retrive_date["paid_by_date"]));?></td>
	...
	public function viewIncomeExpense($id)
	{
		...
		<table class="table table-bordered" width="100%" border="1" style="border-collapse:collapse;">
			...
			{ ?>
				<tr>
				<td><?php echo $i; ?></td>
				<td class="text-center"><?php echo date($this->GYMFunction->getSettings("date_format"),strtotime($data[0]["invoice_date"]));?></td>
