	...
	<div class="col-md-12 box box-default">		
		...
		<?php
			...
			*/
			...
			echo $this->Form->input(__("Instalment Amount"),["name"=>"installment_amount","class"=>"form-control validate[required]","value"=>$membership_data['installment_amount']]);
			echo "</div>";			
			
			echo "<div class='form-group col-md-4'>";
			echo $this->Form->label(__("Instalment Period"));								
			echo $this->Form->select("install_plan_id",$installment_plan,["default"=>$membership_data["install_plan_id"],"empty"=>__("Select Instalment Plan"),"class"=>"form-control plan_list validate[required]"]);
			echo "</div>";			
			...
			echo $this->Form->button(__("Add Instalment Plan"),["class"=>"form-control add_plan btn btn-success btn-flat","type"=>"button","data-url"=>$this->Gym->createurl("GymAjax","addInstalmentPlan")]);
			echo "</div>";
