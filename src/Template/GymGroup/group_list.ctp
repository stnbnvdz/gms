		...
		<div class="box-header">
			<section class="content-header">
			  ...
			  <?php
				...
				{ ?>
			  <ol class="breadcrumb">
				<a href="<?php echo $this->Gym->createurl("GymGroup","addGroup");?>" class="btn btn-flat btn-custom"><i class="fa fa-plus"></i> <?php echo __("Class");?></a>
			  </ol>
			  <?php } ?>
			</section>
		</div>
		...
		<table class="mydataTable table table-striped">
			...
			foreach($data as $row)
			{
				...
				echo "<a href='".$this->Gym->createurl('GymGroup','editGroup')."/{$row['id']}' class='btn btn-flat btn-primary' title='Edit'><i class='fa fa-edit'></i></a>
					<a href='".$this->Gym->createurl('GymGroup','deleteGroup')."/{$row['id']}' class='btn btn-flat btn-danger' title='Delete' onClick=\"return confirm('Are you sure you want to delete?')\"><i class='fa fa-trash-o'></i></a>";
				}
