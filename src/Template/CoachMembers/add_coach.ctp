...
$(document).ready(function() {
	...
	if($edit)
	{?>
	$( ".date" ).datepicker( "setDate", new Date("<?php echo date($this->Gym->getSettings("date_format"),strtotime($data['birth_date'])); ?>" ));
	<?php } ?>
		...
		<div class="box-header">
			<section class="content-header">			
			  <h1>		
				...
				<?php echo $title;?>
				<small><?php echo __("Coach");?></small>
			  </h1>
			  <ol class="breadcrumb">
				<a href="<?php echo $this->Gym->createurl("CoachMembers","CoachList");?>" class="btn btn-flat btn-custom"><i class="fa fa-bars"></i> <?php echo __("Coach List");?></a>
			  </ol>
			</section>
		</div>
		<hr>
...
/** 		
		...
		*/	
			...
			echo $this->Form->button(__("Save Coach"),['class'=>"btn btn-flat btn-primary","name"=>"add_group"]);
			echo $this->Form->end();
			?>				
		</div>	
